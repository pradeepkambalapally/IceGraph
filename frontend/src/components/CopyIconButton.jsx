...
export default function CopyIconButton({ text, className = '', title = 'Copy value' }) {
  ...
  return (
    <button
      ...
      className={`p-1 rounded border border-[#2d3748] bg-[#1a202c]/90 text-slate-500 hover:text-slate-300 hover:border-slate-500 transition-colors cursor-pointer ${className}`}
    >
      {copied ? (
